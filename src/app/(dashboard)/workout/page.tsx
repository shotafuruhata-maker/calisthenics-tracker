      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-500 mb-4">Set weekly goals first to get personalized workout suggestions</p>
            <Link href="/goals">
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{item.exercise.muscle_group?.name}</p>
                      </div>
          ...
          {data.bonusRound?.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm text-gray-500">Almost there! Finish these off</p>
              </div>
              {data.bonusRound.map((item: {
                exercise: { id: string; name: string }
                remaining: number
              }) => (
                <Card key={item.exercise.id} className="border-amber-200 bg-amber-50">
                  <CardContent className="py-3 flex items-center justify-between">
                    <div>
                      <span className="font-medium text-sm">{item.exercise.name}</span>
                      <span className="text-sm text-gray-500 ml-2">— just {item.remaining} reps to go!</span>
                    </div>
                    <Button
                      size="sm"
                      variant="outline"
